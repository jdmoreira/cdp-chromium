...
if [[ -f "$_root_dir/build_finished_$_target_cpu.log" ]] ; then
  ...
  # cdp-always-on: no Apple Developer ID configured. Ad-hoc sign the whole
  # bundle so the arm64 app will actually launch locally (Apple Silicon
  # requires at least an ad-hoc signature). Developer-ID signing and
  # notarization are skipped entirely.
